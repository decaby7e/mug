...
import sys
import os
import urllib3  # TODO: Ensure urllib2->urllib3 conversion works
    ...
    def __setitem__(self, key, value):
        ...
        for attribute in attributeslist:
            for subattr in attribute:
                (attrname, attrvalue) = subattr
                if attrname == key:
                    subattr[1].append(value)
                    return
...
class IPPRequest:
    ...
    def dump(self):
        ...
        if None not in (self.version, self.operation_id):
            mybuffer.append(chr(self.version[0]).encode() + chr(self.version[1]).encode())
            ...
            for attrtype in self.attributes_types:
                for attribute in getattr(self, "_%s_attributes" % attrtype):
                    if attribute:
                        mybuffer.append(
                            chr(self.tagvalues["%s-attributes-tag" % attrtype]).encode()
                        )
                    for (attrname, value) in attribute:
                        nameprinted = 0
                        for (vtype, val) in value:
                            mybuffer.append(chr(self.tagvalues[vtype]).encode())
                            if not nameprinted:
                                mybuffer.append(pack(">H", len(attrname)))
                                mybuffer.append(attrname.encode())
                                nameprinted = 1
                            else:
                            ...
                            if vtype in ("integer", "enum"):
                                ...
                                mybuffer.append(chr(val).encode())
                            else:
                                mybuffer.append(pack(">H", len(val)))
                                mybuffer.append(val.encode())
            mybuffer.append(chr(self.tagvalues["end-of-attributes-tag"]).encode())
        mybuffer.append(self.data.encode())
    ...
    def parseTag(self):
        ...
        if not namelength:
            ...
            posend += namelength
            self._curname = name = self._data[pos2:posend].decode()
        pos2 = posend + 2
...
class CUPS:
    ...
    def identifierToURI(self, service, ident):
        ...
        return "%s/%s/%s" % (self.url.replace("http://", "ipp://"), service, ident)
    ...
    def newRequest(self, operationid=None):
        ...
        if operationid is not None:
            ...
            req.operation["attributes-natural-language"] = (
                "naturalLanguage",
                self.language,
            )
            return req
    ...
    def doRequest(self, req, url=None):
        ...
        """
        # TODO: move this pool out of the function to be reused?
        http = urllib3.PoolManager()

        url = url or self.url
        headers = {"Content-Type": "application/ipp"}
        if self.username:
            headers.update(urllib3.make_headers(basic_auth=f"{self.username}:{self.password}"))

        self.lastError = None
        self.lastErrorMessage = None

        try:
            response = http.request(method="POST", url=url, headers=headers, body=req.dump())
        except (HTTPError, socket.error) as error:
            self.lastError = error
            self.lastErrorMessage = str(error)
            return None
        finally:
            ipp_response = IPPRequest(response.data)
            ipp_response.parse()
            return ipp_response
    ...
    def getPPD(self, queuename):
        ...
        req.operation["printer-uri"] = (
            "uri",
            self.identifierToURI("printers", queuename),
        )
    ...
    def getDevices(self):
        ...
        return zip(
            [d[1] for d in answer.printer["device-class"]],
    ...
    def getPPDs(self):
        ...
        return zip(
            [d[1] for d in answer.printer["ppd-natural-language"]],
    ...
    def createSubscription(
        ...
        if userdata is not None:
            req.subscription["notify-user-data"] = (
        ...
        if naturallanguage is not None:
            req.subscription["notify-natural-language"] = (
                "naturalLanguage",
                naturallanguage,
            )
...
if __name__ == "__main__":
    if (len(sys.argv) < 2) or (sys.argv[1] == "--debug"):
        ...
        if filedata == filedata2:
            print(
                "Test OK : parsing original and parsing the output of the dump produce the same dump !"
            )
            print(str(msg))
        else:
            print("Test Failed !")
